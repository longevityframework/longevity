...
object LongevityContext {
   ...
   */
  def apply[F[_] : Effect, M : ModelType](typesafeConfig: Config = ConfigFactory.load())
  : LongevityContext[F, M] =
    new LongevityContext(LongevityConfig.fromTypesafeConfig(typesafeConfig))
...
extends PersistenceContext[F, M] with TestContext[F, M] with JsonContext {
  ...
  def this(typesafeConfig: Config)(implicit effect: Effect[F], modelType: ModelType[M]) =
    this(LongevityConfig.fromTypesafeConfig(typesafeConfig))
  ...
  lazy val testDataGenerator = TestDataGenerator[M]
