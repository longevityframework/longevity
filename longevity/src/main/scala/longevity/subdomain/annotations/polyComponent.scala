 ...
 * [[longevity.subdomain.PolyCType PolyCType]]. if the trait already has a
...
private object polyComponent {
  ...
  private abstract class PolyComponentImpl extends AbstractComponentImpl {
    ...
    private lazy val name0 = as.head match {
      ...
      case _ => 
        c.abort(c.enclosingPosition, s"@longevity.subdomain.polyComponent can only be applied to traits")
    }

    protected def ctype = tq"longevity.subdomain.PolyCType[$typeName]"
