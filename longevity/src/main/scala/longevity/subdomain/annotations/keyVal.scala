...
object keyVal {
  ...
  private abstract class KeyValImpl {
    ...
    private def extended: c.Tree = {
      def newPs(ps: Seq[c.Tree]) = ps :+ tq"longevity.subdomain.KeyVal[$ptype]"
      as.head match {
        case q"$ms class $n[..$tps] $cms(...$pss) extends {..$eds} with ..$ps          { $s => ..$ss }" =>
             q"$ms class $n[..$tps] $cms(...$pss) extends {..$eds} with ..${newPs(ps)} { $s => ..$ss }"
        case _ =>
          c.abort(c.enclosingPosition, s"@longevity.subdomain.keyVal can only be applied to a class")
      }
    }
    ...
    private def ptype = c.prefix.tree match {
      ...
      case _ => 
          c.abort(c.enclosingPosition, s"@longevity.subdomain.keyVal must take a single type parameter")
    }
