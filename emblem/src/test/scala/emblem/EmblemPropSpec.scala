...
class EmblemPropSpec extends FlatSpec with GivenWhenThen with Matchers {
  ...
  it should "retain type information" in {
    PointEmblem.x.typeTag should equal (typeTag[Double])
    PointEmblem.y.typeTag should equal (typeTag[Double])
  }
  ...
  it should "retain embedded type information" in {
    PolygonEmblem.corners.typeTag should equal (typeTag[Set[Point]])

    val typeArgs = PolygonEmblem.corners.typeTag.tpe.typeArgs
