...
package object emblem {
  ...
  /** returns a [[TypeKey]] for the specified type `A`. this method will only work where a scala-reflect
   * `TypeTag` is implicitly available. */
   ...
   * anywhere that the corresponding scala-reflect `TypeTag` is implicitly available. */
