...
/** this exception is thrown when a user tries to generate an [[emblem.Emblem
 * Emblem]] or an [[emblem.Extractor Extractor]] for a type that is not a case
 * class.
 *
 * `Emblem` and `Extractor` generation for non-case class types may be supported
