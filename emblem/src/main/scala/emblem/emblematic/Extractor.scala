...
import emblem.emblematic.factories.ExtractorFactory
 ...
 * all values in the range map back onto the domain. the `Range` type is
 * typically a richer type, such as a case class with a single parameter, and
 * the `Domain` type is the type wrapped by the case class.
 *
 ...
 * @param inverse a function to convert from range to domain. throws exception
 * if there is no element in the domain that corresponds to the element in the
 * range
 */
case class Extractor[Domain : TypeKey, Range : TypeKey] private[emblem] (
  ...
  /** a function to convert from range to domain. returns `None` if there is no
   * element in the domain that corresponds to the element in the range
   */
